...
from pypinyin import pinyin, Style  # 添加 pypinyin 导入
...
def run(rank, n_gpus, hps):
  ...
  if rank == 0:
    ...
    logger.info(hps)
    logger.info(f"Loaded symbols (first 10): {symbols[:10]}")  # 调试符号表
  ...
  if rank == 0:
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(eval_loader):
        logger.info(f"Sample text (first 5): {x[:5]}")
        break
  ...
  if hps.cont:
      try:
          ...
          print("Failed to find latest checkpoint, training from scratch...")
          epoch_str = 1
          global_step = 0
  else:
  ...
  for epoch in range(epoch_str, hps.train.epochs + 1):
    ...
    if rank == 0:
      if hps.preserved > 0:
        ...
        old_g = utils.oldest_checkpoint_path(hps.model_dir, "G_[0-9]*.pth", preserved=hps.preserved)
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    net_g.train()
    net_d.train()
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in tqdm(enumerate(train_loader), total=len(train_loader), disable=rank!=0):
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            y = commons.slice_segments(y, ids_slice, hps.train.segment_size)
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                scalar_dict = {"loss/g/total": loss_gen_all, "loss/d/total": loss_disc_all,
                              "learning_rate": lr, "grad_norm_d": grad_norm_d,
